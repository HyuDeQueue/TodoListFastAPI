...
from src.core.constants import RoleUser
...
def add_group_member(db: Session, group_member_data: GroupMemberBase) -> GroupMemberResponse:
    ...
    if not exist_group_member:
        group_member = GroupMember(group_id=GroupMemberBase.group_id, user_id=group_member_data.user_id, role=RoleUser.ADMIN.value)
        db.add(group_member)
        db.commit()
        ...
        group_member = GroupMember(group_id=GroupMemberBase.group_id, user_id=group_member_data.user_id, role=RoleUser.MEMBER.value)
        db.add(group_member)
        db.commit()
        db.refresh(group_member)
        return GroupMemberResponse.model_validate(group_member)
